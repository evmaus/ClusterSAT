...
void UnitPropagate(const ClauseDatabase& terms, 
...
{
  stats.StartBCP();
  auto unit_terms = terms.unit_terms(env);
  while(!unit_terms.empty()) {
    ...
    unit_terms = terms.unit_terms(env);
  }
  stats.EndBCP();
}
...
void PickBranchVariableAndAssign(uint32_t& current_decision_level, const ClauseDatabase& term,
  const std::unique_ptr<VariableSelector>& variable_selector,
  VariableEnvironmentStack& env_stack, 
  CDCLTrace& trace,
  Stats& stats
)
{
  ...
  if (!term.has_empty(env_stack)) {
    ...
    VariableRecommendation next_var = env_stack.next_unbound();
    ...
    env_stack.assign(next_var.first, next_var.second);
  }
    ...
    const cnf::And& term, std::atomic_bool& run) const 
{
  ...
  UnitPropagate(clause_db, env_stack, 0, trace, stats);
  ...
  while (run){
    ...
    else 
    {
      ...
      UnitPropagate(clause_db, env_stack, current_decision_level, trace, stats);
      PickBranchVariableAndAssign(current_decision_level, term, selector, env_stack, trace, stats);
      UnitPropagate(clause_db, env_stack, current_decision_level, trace, stats);

      if (clause_db.has_empty(env_stack)) {
        stats.Conflict();
        LOG(LogLevel::VERBOSE, "Empty term: " + clause_db.next_empty(env_stack).to_string());
